...
pipeline {
    ...
    stages {
        ...
        stage ('SonarQube Analysis') {
            steps {
                withSonarQubeEnv('Wemove SonarQube') {
                    sh "${SCANNER_HOME}/bin/sonar-scanner -Dsonar.projectKey=ingrid-geo-api"
                }
            }
        }
        ...
        stage('Build RPM') {
            ...
            steps {
                script {
                    sh "sed -i 's/^Version:.*/Version: ${determineVersion()}/' rpm/ingrid-geo-api.spec"
                    sh "sed -i 's/^Release:.*/Release: ${determineRpmReleasePart()}/' rpm/ingrid-geo-api.spec"

                    // Prepare build
                    sh "mkdir -p ./build/rpms /root/rpmbuild/SPECS"
                    sh """
                        cp ${WORKSPACE}/rpm/ingrid-geo-api.spec /root/rpmbuild/SPECS/ingrid-geo-api.spec &&
                        rpmbuild -bb /root/rpmbuild/SPECS/ingrid-geo-api.spec
                    """
                    ...
                    withCredentials([
                        ...
                        archiveArtifacts artifacts: "build/rpms/ingrid/ingrid-geo-api-*.rpm", fingerprint: true
                    }
                    ...
                    withCredentials([
                        ...
                        archiveArtifacts artifacts: 'build/rpms/itzbund/ingrid-geo-api-*.rpm', fingerprint: true
                    }
                }
            }
        }
